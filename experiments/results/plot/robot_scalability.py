...
path_to_robot_results_a = ['robot_scalability_1', 'robot_scalability_2', 'robot_scalability_3', 'robot_scalability_4',
                           'robot_scalability_5']
path_to_robot_results_b = ['robot_scalability_1_1', 'robot_scalability_2_1', 'robot_scalability_3_1',
                           'robot_scalability_4_1', 'robot_scalability_5_1']
xticks = ['#robots=1', '#robots=2', '#robots=3', '#robots=4', '#robots=5']


def get_path_to_robot_results(dir_name):
    if dir_name == 'robot_scalability_a':
        return path_to_robot_results_a
    elif dir_name == 'robot_scalability_b':
        return path_to_robot_results_b


def plot_allocations(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"

    save_in_path = get_plot_path(dir_name)
    path_to_robot_results = get_path_to_robot_results(dir_name)
...
def plot_unallocated_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    ...
    for i, approach in enumerate(approaches):
        print("Approach: ", approach)
        avgs_unallocations = list()
        stdevs_unallocations = list()
        ...
        for r in path_to_robot_results:
            ...
            robot_unallocations = list()

            for run_id, run_info in results.get("runs").items():
                print("run_id: ", run_id)
                metrics = run_info.get("performance_metrics").get("fleet_performance_metrics")
                robot_unallocations.append(len(metrics.get("unallocated_tasks")))

            try:
                avg_unallocations = statistics.mean(robot_unallocations)
            except statistics.StatisticsError:
                # mean requires at least one data point
                avg_unallocations = 0

            try:
                stdev_unallocations = statistics.stdev(robot_unallocations)
            except statistics.StatisticsError:
                # variance requires at least two data points
                stdev_unallocations = 0

            avgs_unallocations.append(avg_unallocations)
            stdevs_unallocations.append(stdev_unallocations)

        plt.errorbar(robots, avgs_unallocations, stdevs_unallocations, marker=markers[i], label=ticks[i])
...
def plot_reallocated_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"

    save_in_path = get_plot_path(dir_name)
    path_to_robot_results = get_path_to_robot_results(dir_name)
    plot_name = "reallocated_tasks"
    fig = plt.figure(figsize=(9, 6))
    robots = list(range(1, 6))

    for i, approach in enumerate(approaches):
        print("Approach: ", approach)
        avgs_reallocations = list()
        stdevs_reallocations = list()
        ...
        for r in path_to_robot_results:
            ...
            robot_reallocations = list()

            for run_id, run_info in results.get("runs").items():
                print("run_id: ", run_id)
                metrics = run_info.get("performance_metrics").get("fleet_performance_metrics")
                robot_reallocations.append(len(metrics.get("re_allocated_tasks")))

            try:
                avg_reallocations = statistics.mean(robot_reallocations)
            except statistics.StatisticsError:
                # mean requires at least one data point
                avg_reallocations = 0

            try:
                stdev_reallocations = statistics.stdev(robot_reallocations)
            except statistics.StatisticsError:
                # variance requires at least two data points
                stdev_reallocations = 0

            avgs_reallocations.append(avg_reallocations)
            stdevs_reallocations.append(stdev_reallocations)

        plt.errorbar(robots, avgs_reallocations, stdevs_reallocations, marker=markers[i], label=ticks[i])
...
def plot_unsuccessfully_reallocated_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"

    save_in_path = get_plot_path(dir_name)
    path_to_robot_results = get_path_to_robot_results(dir_name)
    plot_name = "unsuccessfully_reallocated_tasks"
    fig = plt.figure(figsize=(9, 6))
    robots = list(range(1, 6))

    for i, approach in enumerate(approaches):
        print("Approach: ", approach)
        avgs_unsucessful_reallocations = list()
        stdevs_unsuccessful_reallocations = list()
        ...
        for r in path_to_robot_results:
            ...
            robot_unsuccessful_reallocations = list()

            for run_id, run_info in results.get("runs").items():
                print("run_id: ", run_id)
                metrics = run_info.get("performance_metrics").get("fleet_performance_metrics")
                robot_unsuccessful_reallocations.append(len(metrics.get("unsuccessfully_re_allocated_tasks")))

            try:
                avg_unsuccessful_reallocations = statistics.mean(robot_unsuccessful_reallocations)
            except statistics.StatisticsError:
                # mean requires at least one data point
                avg_unsuccessful_reallocations = 0

            try:
                stdev_unsuccessful_reallocations = statistics.stdev(robot_unsuccessful_reallocations)
            except statistics.StatisticsError:
                # variance requires at least two data points
                stdev_unsuccessful_reallocations = 0

            avgs_unsucessful_reallocations.append(avg_unsuccessful_reallocations)
            stdevs_unsuccessful_reallocations.append(stdev_unsuccessful_reallocations)

        plt.errorbar(robots, avgs_unsucessful_reallocations, stdevs_unsuccessful_reallocations, marker=markers[i], label=ticks[i])
...
def box_plot_allocations(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_re_allocations(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_re_allocation_attempts(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_successful_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    ...
    plt.yticks(list(range(0, 26, 5)))
    plt.xlabel("Number of robots")
    plt.ylabel("Average number of on-time tasks")
    # plt.title(title)
    axes = plt.gca()
    axes.yaxis.grid()
    lgd = axes.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1), ncol=4, fancybox=True, shadow=True)
...
def box_plot_successful_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    ...
    ax.set_ylabel('Average number of successful tasks')
    ax.yaxis.grid()
...
def box_plot_completed_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_completed_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_delayed_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    ...
    for i, approach in enumerate(approaches):
        ...
        for r in path_to_robot_results:
            ...
            robot_delayed_tasks = list()

            for run_id, run_info in results.get("runs").items():
                print("run_id: ", run_id)
                metrics = run_info.get("performance_metrics").get("fleet_performance_metrics")
                robot_delayed_tasks.append(len(metrics.get("delayed_tasks")))

            try:
                avg_delayed_tasks = statistics.mean(robot_delayed_tasks)
            except statistics.StatisticsError:
                # mean requires at least one data point
                avg_delayed_tasks = 0

            try:
                stdev_delayed_tasks = statistics.stdev(robot_delayed_tasks)
            except statistics.StatisticsError:
                # variance requires at least two data points
        ...
        plt.errorbar(robots, avgs_delayed_tasks, stdevs_delayed_tasks, marker=markers[i], label=ticks[i])
...
def plot_early_tasks(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    ...
    for i, approach in enumerate(approaches):
        ...
        for r in path_to_robot_results:
            ...
            robot_early_tasks = list()

            for run_id, run_info in results.get("runs").items():
                print("run_id: ", run_id)
                metrics = run_info.get("performance_metrics").get("fleet_performance_metrics")
                robot_early_tasks.append(len(metrics.get("early_tasks")))

            try:
                avg_early_tasks = statistics.mean(robot_early_tasks)
            except statistics.StatisticsError:
                # mean requires at least one data point
                avg_early_tasks = 0

            try:
                stdev_early_tasks = statistics.stdev(robot_early_tasks)
            except statistics.StatisticsError:
                # variance requires at least two data points
                stdev_early_tasks = 0
        ...
        plt.errorbar(robots, avgs_early_tasks, stdevs_early_tasks, marker=markers[i], label=ticks[i])
...
def plot_allocation_times(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_dgraph_recomputation_times(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_re_allocation_times(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_robot_utilization(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_bid_time_vs_tasks_in_schedule(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
    path_to_robot_results = get_path_to_robot_results(dir_name)
    ...
    for r in path_to_robot_results:
        ...
        plt.xlabel("Number of tasks in robot's schedule")
        plt.ylabel("Time [s]")
        # plt.title(title)
        axes = plt.gca()
        axes.yaxis.grid()
...
def plot_re_allocation_per_task_info(approaches, dir_name):
    title = "Experiment: Robot scalability \n" + \
            "Recovery method: re-allocation \n"
...
def plot_amount_of_delay_and_earliness(approaches, dir_name):
    save_in_path = get_plot_path(dir_name)
    path_to_robot_results = get_path_to_robot_results(dir_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("dir_name", type=str, action='store', help='Name of dir where plots will be stored',
                        default='robot_scalability_a')
    config_params = get_config_params(experiment='robot_scalability_1')
    approaches = config_params.get("approaches")
    args = parser.parse_args()

    # box_plot_allocations(approaches, args.dir_name)
    # box_plot_completed_tasks(approaches, args.dir_name)
    # box_plot_successful_tasks(approaches, args.dir_name)

    # plot_allocations(approaches, args.dir_name)
    # plot_completed_tasks(approaches, args.dir_name)
    # plot_successful_tasks(approaches, args.dir_name)
    # plot_early_tasks(approaches, args.dir_name)
    # plot_delayed_tasks(approaches, args.dir_name)
    #
    # plot_unallocated_tasks(approaches, args.dir_name)
    # plot_reallocated_tasks(approaches, args.dir_name)
    # plot_unsuccessfully_reallocated_tasks(approaches, args.dir_name)

    # plot_amount_of_delay_and_earliness(approaches, args.dir_name)
    #
    # plot_re_allocations(approaches, args.dir_name)
    # plot_re_allocation_attempts(approaches, args.dir_name)
    # plot_re_allocation_times(approaches, args.dir_name)
    #
    # plot_allocation_times(approaches, args.dir_name)
    # plot_dgraph_recomputation_times(approaches, args.dir_name)
    plot_robot_utilization(approaches, args.dir_name)
    # plot_bid_time_vs_tasks_in_schedule(approaches, args.dir_name)
    #
    # plot_re_allocation_per_task_info(approaches, args.dir_name)

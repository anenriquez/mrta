...
from mrs.experiments.db.models.experiment import Experiment as ExperimentModel
...
class PerformanceMetrics(AsDictMixin):
    def __init__(self, n_tasks, start_time, tasks_by_robot, n_allocated_tasks, n_re_allocation_attempts,
                 ...
                 fleet_completion_time, fleet_makespan, fleet_travel_time, fleet_work_time, fleet_idle_time,
        ...
        self.fleet_completion_time = fleet_completion_time
...
class Run(AsDictMixin):
    ...
    def get_performance_metrics(self):
        ...
        fleet_completion_time = (finish_time - start_time).total_seconds()
        fleet_travel_time = self.get_fleet_travel_time(fleet_completion_time)
        fleet_work_time = self.get_fleet_work_time(fleet_completion_time)
        ...
        metrics = {"n_tasks": self._n_tasks,
                   ...
                   "fleet_completion_time": fleet_completion_time,
    ...
    def get_fleet_travel_time(self, fleet_completion_time):
        travel_time = 0
        for performance in self._run_info.robots_performance:
            travel_time += performance.travel_time
        return 100 * travel_time/(fleet_completion_time*len(self._robot_ids))

    def get_fleet_work_time(self, fleet_completion_time):
        work_time = 0
        for performance in self._run_info.robots_performance:
            work_time += performance.work_time
        return 100 * work_time/(fleet_completion_time*len(self._robot_ids))

    def get_fleet_robot_usage(self):
        n_robots_used = 0
        for performance in self._run_info.robots_performance:
            if performance.completion_time > 0.0:
                n_robots_used += 1

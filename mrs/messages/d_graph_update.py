...
class DGraphUpdate(AsDictMixin):
    ...
    def update_timetable(self, timetable, last_task, replace=False):
        ...
        if replace:
            ...
            merged_stn = self.merge_temporal_graph(timetable.stn, stn, last_task)
            merged_dispatchable_graph = self.merge_temporal_graph(timetable.dispatchable_graph, dispatchable_graph, last_task)
    ...
    @staticmethod
    def merge_temporal_graph(previous_graph, new_graph, last_task):
        tasks = list()
        new_task_ids = new_graph.get_tasks()

        scheduled_tasks = [task.task_id for task in Task.get_tasks_by_status(TaskStatusConst.SCHEDULED)]
        ongoing_tasks = [task.task_id for task in Task.get_tasks_by_status(TaskStatusConst.ONGOING)]
        do_not_update = scheduled_tasks + ongoing_tasks
        if last_task is not None:
            do_not_update.append(last_task.task_id)

        print("do not update: ", do_not_update)

        for i, task_id in enumerate(new_task_ids):
            if task_id in do_not_update:
                print("task {} is ongoing, dot not update".format(task_id))
                # Keep current version of task
                tasks.append(previous_graph.get_task_graph(task_id))
            else:
                # Use new version of task
                print("Use new version of task: ", task_id)
